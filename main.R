      ...
      for(l in 1:length(beliefSets)){
        ...
        for(m in 1:nrow(q1)){
          ...
          if(length(forecast)>0){
            tbl = rbind(tbl, data.frame(userType = q1[m,]$group,
                                        forecast = forecast,
                                        quintile = "Q1"))
          }
        }
        ...
        for(m in 1:nrow(q2)){
          ...
          forecast = (csv %>% filter(userId == currentId) %>% select(forecast))$forecast
          if(length(forecast)>0){
            tbl = rbind(tbl, data.frame(userType = q2[m,]$group,
                                        forecast = forecast,
                                        quintile = "Q2"))
          }
        }
        ...
        for(m in 1:nrow(q3)){
          ...
          forecast = (csv %>% filter(userId == currentId) %>% select(forecast))$forecast
          if(length(forecast)>0){
            tbl = rbind(tbl, data.frame(userType = q3[m,]$group,
                                        forecast = forecast,
                                        quintile = "Q3"))
          }
        }
        ...
        for(m in 1:nrow(q4)){
          ...
          forecast = (csv %>% filter(userId == currentId) %>% select(forecast))$forecast
          if(length(forecast)>0){
            tbl = rbind(tbl, data.frame(userType = q4[m,]$group,
                                        forecast = forecast,
                                        quintile = "Q4"))
          }
        }
        ...
        for(m in 1:nrow(q5)){
          ...
          forecast = (csv %>% filter(userId == currentId) %>% select(forecast))$forecast
          if(length(forecast)>0){
            tbl = rbind(tbl, data.frame(userType = q5[m,]$group,
                                        forecast = forecast,
                                        quintile = "Q5"))
          }
        }
